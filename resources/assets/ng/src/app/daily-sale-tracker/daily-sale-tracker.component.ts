...
export class DailySaleTrackerComponent implements OnInit {
  ...
  tableEmpty:boolean;
  floatIsOpen:BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  form:FormGroup;
  ...
  ngOnInit() {
    ...
    this.userService.user.subscribe(u => {
      this.userInfo = u;
      this.clientService.getSaleStatuses(this.userInfo.selectedClient.clientId)
        .subscribe(statuses => {
          this.store.statuses = statuses;
          this.statuses.next(statuses);
        });

      this.campaignService.getCampaigns(this.userInfo.selectedClient.clientId)
        .then(campaigns => {
    ...
    this.agentService.getAgents(true)
      .then(agents => {
        this.agents = agents;
      })
  ...
  showAddSaleDialog():void {
    this.dialog.open(AddSaleDialog, {
      width: '800px',
      data: {
    ...
    .subscribe((result:any) => {
      ...
      this.trackerService.createDailySale(dto.clientId, dto)
        .subscribe(() => {
          this.refreshDailySales(this.startDate, this.endDate);
        });
    });
  ...
  changeSelectedCampaignId(event:MatSelectChange):void {
    this.selectedCampaign = _.find(this.campaigns, { 'campaignId': event.value });
  ...
  getSalesControl(name:string, index:number):FormControl {
    return this.form.get('sales').get(index+'').get(name) as FormControl;
  }
  ...
  editExistingSale(index:number):void {
    ...
    .subscribe(dto => {
      ...
      this.trackerService.updateDailySale(this.userInfo.selectedClient.clientId, dto)
        .subscribe(sale => {
  ...
  deleteSale(index:number):void {
    ...
    this.dialog.open(DeleteSaleDialog, {
      width: '300'
    })
    .afterClosed()
    .subscribe(result => {
      ...
      this.trackerService.deleteDailySale(this.userInfo.selectedClient.clientId, dto.dailySaleId)
        .subscribe(() => {
  ...
  private refreshDailySales(startDate:Moment, endDate:Moment):void {
    ...
    this.trackerService.getDailySalesByDate(
  ...
  private setActivityDateProperty(formPaidStatus:PaidStatusType, modelField:string, index:number):Date|string|Moment {
