...
export class AddAgentDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.agentService.getAgents()
      .then(agents => {
        this.managers = _.filter(agents, (a:IAgent) => { return a.isManager; });
      })
  ...
  saveNewUserAgentEntity():void {
    ...
    this.userService.saveNewUserAgentEntity(this.userEntity, this.agentEntity, this.detailEntity)
      .then(() => {
  ...
  private createUserForm():void {
    let pw = this.makeUpperCaseRandomLetter(Math.random().toString(36).slice(-8));
  ...
  private createAgentForm():void {
    this.userEntity = this.userEntity == null
      ? <IUser>{
        firstName: null,
        lastName: null
      }
