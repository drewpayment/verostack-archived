...
export class AgentsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.session.userItem.subscribe((u:IUser) => {
      ...
      this.agentService.getAgents(false)
        .then((agents:IAgent[]) => {
          this.managers = [];
      ...
      this.campaignService.getCampaigns(u.selectedClient.clientId)
        .then(campaigns => {
          this.campaigns = [];
          for(let i = 0; i < campaigns.length; i++) {
            if(campaigns[i].clientId != this.user.selectedClient.clientId
              && !campaigns[i].active) continue;
  ...
  editAgentDetail(agent:IAgent):void {
    ...
    this.agentService.getAgentSalesPairings(agent.agentId, this.user.selectedClient.clientId)
      .then(pairings => {
        this.salesPairings = pairings;
      })
      .catch(this.msg.showWebApiError);

    this.userService.getUser(agent.userId)
      .then(user => {
        this.editingUser = user;
        this.userService.getUserDetailById(this.editingUser.id)
          .then(detail => {
  ...
  handleUserTabSave():void {
    ...
    if(agentForm.dirty && agentForm.valid)
      this.updateFormAgentInfo(agentForm, userForm);
  }
  ...
  updateFormSaleInfo():void {
    ...
    if(saleInfo.value.saleOneCampaignId && !saleInfo.value.saleOneId
    ...
    this.userService.updateDetailSaleInfo(this.detail)
      .then(detail => {
  ...
  updateAgentSalesPairings():void {
    ...
    this.agentService.saveAgentSalesPairings(payLoad, this.editingAgent.agentId)
      .then(pairings => {
  ...
  removeAgentSalesPairing(pairing:ISalesPairing) {
    this.session.showLoader();
    let idx:number = this.salesPairings.map((p:ISalesPairing) =>
      { return p.salesPairingsId; }).indexOf(pairing.salesPairingsId);

    this.agentService.deleteAgentSalesPairings(pairing.salesPairingsId)
      .then(() => {
  ...
  updateFormDetailInfo():void {
    ...
    this.userService.saveDetailEntity(detail)
      .then(detail => {
        this.setViewValues(detail);
  ...
  getCampaignName(campaignId:number):string {
    return (<ICampaign>_.find(this.campaigns, (c:ICampaign) => {
  ...
  private resetAgentInfo(agent:IAgent):void {
    this.detailForm.setControl('agentInfo', this.fb.group({
  ...
  private resetUserInfoForm(user:IUser):void {
    this.detailForm.setControl('userInfo', this.fb.group({
      username: this.fb.control(this.user.username || '', [Validators.required]),
      email: this.fb.control(this.user.email || '', [Validators.required]),
      canLogin: this.fb.control(this.user.active || '')
    }));
  ...
  private resetDetailInfoForm(detail:IUserDetail):void {
    this.detailForm.setControl('detailInfo', this.fb.group({
      street: this.fb.control(this.detail.street || '', [Validators.required]),
      street2: this.fb.control(this.detail.street2 || ''),
      city: this.fb.control(this.detail.city || '', [Validators.required]),
      state: this.fb.control(this.detail.state || '', [Validators.required]),
  ...
  private resetSaleInfoForm(detail:IUserDetail):void {
    this.detailForm.setControl('saleInfo', this.fb.group({
